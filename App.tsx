...
import { RefreshCw, Menu, CloudOff, Cloud, AlertCircle, ShieldCheck, User, Lock, Cpu } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const fetchCloudData = async () => {
    ...
    try {
      const [
        { data: cItems, error: e1 }, 
        { data: cUsers, error: e2 }, 
        { data: cSuppliers, error: e3 }, 
        { data: cTrans, error: e4 }
      ] = await Promise.all([
      ...
      if (e1 || e2 || e3 || e4) throw new Error("Gagal sinkronisasi data.");
      ...
      if (isDelete) {
        ...
        const dbPayload = payload.map(item => {
          ...
          Object.keys(item).forEach(key => {
            entry[key.toLowerCase()] = item[key];
          });
          return entry;
        });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-tr from-[#020617] via-[#1e3a8a] to-[#020617] flex flex-col items-center justify-center p-6 text-white font-sans overflow-hidden relative">
        ...
        <div className="w-full max-w-[420px] animate-in fade-in zoom-in duration-700 relative z-10">
          ...
          <div className="mt-12 text-center space-y-3 opacity-40">
            <p className="text-[10px] text-blue-100 font-black uppercase tracking-[0.3em]">MAXIMA CLINICAL LABORATORY</p>
            <p className="text-[9px] text-blue-200 font-bold uppercase tracking-widest">VERIFIED SYSTEM v5.3 PRODUCTION</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex bg-[#f8fafc] font-sans relative overflow-hidden h-screen">
      ...
      <div className={`flex-1 flex flex-col h-screen overflow-hidden transition-all duration-300 ease-in-out ${isSidebarOpen && window.innerWidth >= 1024 ? 'pl-64' : 'pl-0'}`}>
        <header className="bg-white/80 backdrop-blur-md px-6 py-4 flex justify-between items-center border-b border-slate-200 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4">
             {lastError && (
              <div className="hidden md:flex bg-rose-50 text-rose-600 px-4 py-2 rounded-xl text-[10px] font-black items-center gap-2 border border-rose-100 animate-pulse">
        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 pointer-events-none">
          {isSyncing && (
            <div className="px-5 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase flex items-center gap-3 shadow-2xl border border-white/10 animate-in slide-in-from-right duration-300">
              <RefreshCw className="w-3 h-3 animate-spin text-blue-400" /> Sinkronisasi...
            </div>
          )}
          <div className={`px-5 py-3 rounded-2xl text-[10px] font-black uppercase flex items-center gap-3 shadow-2xl border pointer-events-auto transition-all duration-500 ${dbStatus === 'CONNECTED' ? 'bg-white/90 backdrop-blur-md text-emerald-600 border-emerald-100' : 'bg-rose-600 text-white border-rose-500'}`}>
            {dbStatus === 'CONNECTED' ? <Cloud className="w-4 h-4" /> : <CloudOff className="w-4 h-4" />} 
            {dbStatus === 'CONNECTED' ? 'Jaringan Online' : 'Mode Offline'}
          </div>
        </div>
      </div>
    </div>
